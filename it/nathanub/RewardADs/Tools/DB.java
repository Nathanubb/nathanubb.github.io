...
public class DB {
		...
		for(Map.Entry<String, Map<String, String>> entry : config.entrySet()) {
            ...
            if(entry.getKey().contains(player.getDisplayName())) {
            	for(Map.Entry<String, String> dataEntry : data.entrySet()) {
            		if(dataEntry.getKey().contains(new ReadInformations().getCode())) {
            			data.put(new ReadInformations().getCode(), coins + "");
            		}
            	}
            }
        }
	...
	public void setEarnPerAD() {
		...
		for(Map.Entry<String, Map<String, String>> entry : config.entrySet()) {
            ...
            if(entry.getKey().contains(new ReadInformations().getCode())) {
            	for(Map.Entry<String, String> dataEntry : data.entrySet()) {
            		if(dataEntry.getKey().contains("coins-per-ad")) {
            			data.put("coins-per-ad", new ReadInformations().getEarnPerAD() + "");
            		}
            	}
            }
        }
	...
	private void writeDatabase(String databaseName, String config) {
        try {
        	String url = "http://rewardads.vpsgh.it:1209";
        	String password = "bj,f678nf?>(rd673)";
	...
	private String fetchDatabase(String databaseName) {
		...
		try {
			String url = "http://rewardads.vpsgh.it/database/" + databaseName + ".txt";
